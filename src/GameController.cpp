...
bool GameController::draculaAbilityAvailable() const {
    if (!started_ || gameOver_ || draculaAbilityUsed_) {
        return false;
    }
    const Fighter& hero = currentPlayer().heroFighter();
    if (dynamic_cast<const Dracula*>(&hero) == nullptr) {
        return false;
    }
    if (hero.defeated()) {
        return false;
...
std::vector<int> GameController::valuesOnCard(const Card& card) const {
    ...
    if (card.getAttack() >= 0) {
        values.push_back(card.getAttack());
    }
    if (card.getDefense() >= 0) {
        values.push_back(card.getDefense());
    }
...
bool GameController::cardEffectsProtectedBySherlock(const Card& card, const Player& owner) const {
    ...
    if (dynamic_cast<const Sherlock*>(&hero) == nullptr) {
        return false;
    }
...
std::map<int, int> GameController::computeReachableWithCost(int start, int maxSteps, const std::string& fighterId) const {
    ...
    if (fighter) {
        ...
        if (invisible) {
            fogSpaces = invisible->getFogSpaces();
        }
    }
    ...
    while (!dq.empty()) {
        ...
        if (invisible && invisible->isOnFog(current)) {
            for (int fogSpace : fogSpaces) {
                ...
                int moveCost = 1;
                if (isSpaceOccupiedByAlly(fogSpace, fighterId)) {
                    moveCost = 2;
                }
                int newCost = currentCost + moveCost;
                if (newCost > maxSteps) continue;
...
void GameController::startNewGame(int playerOneAge, int playerTwoAge, 
    ...
    pendingOptionalMovements_.clear();
    pendingVanishedPlacement_ = false;
    vanishedPlayerIndex_ = -1;
    pendingDeckTopSelection_.reset();
    pendingFogChoices_.clear();
    pendingFogTeleport_.reset();
    pendingConfoundDecision_.reset();
    pendingConfoundDiscardPlayerIndex_ = -1;

    refreshTurnStartFogStatus();
}
...
void GameController::endTurnIfNeeded() {
    ...
    if (pendingVanishedPlacement_) return;
    if (hasPendingDeckTopSelection()) return;
    if (hasPendingFogChoice()) return;
    if (hasPendingFogTeleport()) return;
    if (hasPendingConfoundDecision() || hasPendingConfoundDiscardSelection()) return;
    if (actionsRemaining_ == 0 && currentPlayerMustDiscardToLimit()) {
        return;
    }
    if (actionsRemaining_ == 0) {
...
void GameController::forceEndActionsThisTurn() {
    actionsRemaining_ = 0;
}
...
Player& GameController::mutableOwnerOfFighter(const std::string& fighterId) {
    return ownerOfFighterMutable(fighterId);
}

Player& GameController::mutableOpponentOfFighter(const std::string& fighterId) {
    const Player* owner = ownerOfFighter(fighterId);
    if (!owner) throw RuleViolation("Unknown fighter.");
    return playerByIndex(owner->id() == 0 ? 1 : 0);
}
...
bool GameController::isCardPlayableBy(const Card& card, const Fighter& fighter) const {
    ...
    if (card.getOwner() == Character::Sister || card.getOwner() == Character::Watson) {
        const Player* owner = ownerOfFighter(fighter.id());
        if (owner != nullptr && !owner->hasLivingCharacter(card.getOwner())) {
            return false;
        }
    }
    return fighter.canPlayCard(card);
...
void GameController::refreshTurnStartFogStatus() {
    const Fighter& hero = currentPlayer().heroFighter();
    const auto* invisible = dynamic_cast<const InvisibleMan*>(&hero);
    currentTurnStartedOnFog_ = (invisible != nullptr) && invisible->isOnFog(hero.spaceId());
}
...
void GameController::placeVanishedInvisibleMan(int spaceId) {
    if (!pendingVanishedPlacement_ || vanishedPlayerIndex_ == -1) {
        throw RuleViolation("Invisible Man is not waiting to be placed.");
    }
    if (!board_.contains(spaceId)) {
        throw RuleViolation("Invalid space.");
    ...
    Player& player = playerByIndex(vanishedPlayerIndex_);
    Fighter& invisible = player.heroFighter();
    invisible.placeAt(spaceId);
    pendingVanishedPlacement_ = false;
    vanishedPlayerIndex_ = -1;
...
void GameController::queueDeckTopSelection(int playerIndex, int count) {
    PendingDeckTopSelection selection;
    selection.playerIndex = playerIndex;
    selection.remaining = count;
    pendingDeckTopSelection_ = selection;
}

void GameController::selectCardForDeckTop(int handIndex) {
    if (!pendingDeckTopSelection_.has_value() || pendingDeckTopSelection_->remaining <= 0) {
        throw RuleViolation("No pending deck-top selection.");
    }
    Player& player = playerByIndex(pendingDeckTopSelection_->playerIndex);
    Card card = player.removeCardFromHand(handIndex);
    player.deck().push_back(std::move(card)); 
    --pendingDeckTopSelection_->remaining;
    if (pendingDeckTopSelection_->remaining <= 0) {
        pendingDeckTopSelection_.reset();
        endTurnIfNeeded();
    }
}
...
void GameController::queuePendingFogChoice(int chooserPlayerIndex, const std::string& invisibleManFighterId,
                                           std::vector<int> excludedTokenIndexes, int maxSteps) {
    const Player* owner = ownerOfFighter(invisibleManFighterId);
    if (!owner) return;
    PendingFogChoice choice;
    choice.chooserPlayerIndex = chooserPlayerIndex;
    choice.invisibleManOwnerIndex = owner->id();
    choice.invisibleManFighterId = invisibleManFighterId;
    choice.excludedTokenIndexes = std::move(excludedTokenIndexes);
    choice.maxSteps = maxSteps;
    pendingFogChoices_.push_back(std::move(choice));
}

std::vector<int> GameController::fogTokenOptionsForPendingChoice() const {
    if (pendingFogChoices_.empty()) return {};
    const auto& choice = pendingFogChoices_.front();
    const Fighter* fighter = findFighterById(choice.invisibleManFighterId);
    ...
    for (int i = 0; i < static_cast<int>(tokens.size()); ++i) {
        ...
        if (std::find(choice.excludedTokenIndexes.begin(), choice.excludedTokenIndexes.end(), i)
            != choice.excludedTokenIndexes.end()) continue;
        result.push_back(i);
    }
    return result;
}

std::vector<int> GameController::fogTokenReachableSpaces(const std::string& fighterId, int tokenIndex, int maxSteps) const {
    const Fighter* fighter = findFighterById(fighterId);
    const auto* invisible = dynamic_cast<const InvisibleMan*>(fighter);
    if (!invisible) return {};
    const auto& tokens = invisible->getFogTokens();
    if (tokenIndex < 0 || tokenIndex >= static_cast<int>(tokens.size())) return {};
    int start = tokens[tokenIndex];
    if (start == -1) return {};

    
    std::vector<int> occupiedByOtherFog;
    for (const auto& player : players_) {
        const auto* otherInvisible = dynamic_cast<const InvisibleMan*>(&player.heroFighter());
        if (!otherInvisible) continue;
        const auto& otherTokens = otherInvisible->getFogTokens();
        for (int i = 0; i < static_cast<int>(otherTokens.size()); ++i) {
            if (otherInvisible == invisible && i == tokenIndex) continue;
            if (otherTokens[i] != -1) occupiedByOtherFog.push_back(otherTokens[i]);
        }
    }
    ...
    while (!q.empty()) {
        ...
        for (int neighbor : board_.movementNeighbors(current)) {
            if (dist.find(neighbor) != dist.end()) continue;
            dist[neighbor] = d + 1;
            q.push(neighbor);
        }
    }
    ...
    for (const auto& kv : dist) {
        ...
        if (std::find(occupiedByOtherFog.begin(), occupiedByOtherFog.end(), kv.first) != occupiedByOtherFog.end()) continue;
...
void GameController::resolvePendingFogChoice(int tokenIndex, int destinationSpace) {
    if (pendingFogChoices_.empty()) {
        throw RuleViolation("No pending fog choice.");
    }
    PendingFogChoice choice = pendingFogChoices_.front();

    auto options = fogTokenOptionsForPendingChoice();
    if (std::find(options.begin(), options.end(), tokenIndex) == options.end()) {
        throw RuleViolation("Invalid fog token selection.");
    }
    auto reachable = fogTokenReachableSpaces(choice.invisibleManFighterId, tokenIndex, choice.maxSteps);
    if (std::find(reachable.begin(), reachable.end(), destinationSpace) == reachable.end()) {
        throw RuleViolation("Destination is not reachable for this fog token.");
    }

    pendingFogChoices_.pop_front();

    Fighter* fighter = findFighterById(choice.invisibleManFighterId);
    auto* invisible = dynamic_cast<InvisibleMan*>(fighter);
    if (invisible) {
        invisible->moveFogToken(tokenIndex, destinationSpace);
    }

    bool wasOwnersOwnMove = (choice.chooserPlayerIndex == choice.invisibleManOwnerIndex);
    if (wasOwnersOwnMove) {
        std::vector<int> newExcluded = choice.excludedTokenIndexes;
        newExcluded.push_back(tokenIndex);
        int opponentIdx = (choice.invisibleManOwnerIndex == 0) ? 1 : 0;
        queuePendingFogChoice(opponentIdx, choice.invisibleManFighterId, newExcluded, 2);
    }
...
void GameController::queueFogTeleport(int ownerPlayerIndex, const std::string& fighterId) {
    PendingFogTeleport teleport;
    teleport.fighterOwnerPlayerIndex = ownerPlayerIndex;
    teleport.fighterId = fighterId;
    pendingFogTeleport_ = teleport;
}

std::vector<int> GameController::fogTeleportDestinations() const {
    if (!pendingFogTeleport_.has_value()) return {};
    const Fighter* fighter = findFighterById(pendingFogTeleport_->fighterId);
    const auto* invisible = dynamic_cast<const InvisibleMan*>(fighter);
    if (!invisible || !fighter) return {};
    std::vector<int> result;
    for (int space : invisible->getFogSpaces()) {
        if (space != fighter->spaceId()) result.push_back(space);
    }
    return result;
}

void GameController::resolveFogTeleport(int destinationSpace) {
    if (!pendingFogTeleport_.has_value()) {
        throw RuleViolation("No pending fog teleport.");
    }
    auto destinations = fogTeleportDestinations();
    if (std::find(destinations.begin(), destinations.end(), destinationSpace) == destinations.end()) {
        throw RuleViolation("Invalid fog teleport destination.");
    }
    PendingFogTeleport teleport = *pendingFogTeleport_;
    pendingFogTeleport_.reset();
    Fighter* fighter = findFighterById(teleport.fighterId);
...
void GameController::queueConfoundDecision(int cardOwnerPlayerIndex) {
    PendingConfoundDecision decision;
    decision.cardOwnerPlayerIndex = cardOwnerPlayerIndex;
    decision.deciderPlayerIndex = (cardOwnerPlayerIndex == 0) ? 1 : 0;
    pendingConfoundDecision_ = decision;
}

void GameController::resolveConfoundDecision(bool willDiscard) {
    if (!pendingConfoundDecision_.has_value()) {
        throw RuleViolation("No pending Confound decision.");
    }
    PendingConfoundDecision decision = *pendingConfoundDecision_;
    pendingConfoundDecision_.reset();

    if (willDiscard) {
        pendingConfoundDiscardPlayerIndex_ = decision.deciderPlayerIndex;
    } else {
        
        Player& owner = playerByIndex(decision.cardOwnerPlayerIndex);
        const auto* invisible = dynamic_cast<const InvisibleMan*>(&owner.heroFighter());
        if (invisible && !invisible->getFogSpaces().empty()) {
            queuePendingFogChoice(decision.cardOwnerPlayerIndex, owner.heroFighter().id(), {}, 999);
        }
    }

    if (!hasPendingConfoundDiscardSelection() && !hasPendingFogChoice()) {
        endTurnIfNeeded();
    }
}

void GameController::resolveConfoundDiscardSelection(int handIndex) {
    if (pendingConfoundDiscardPlayerIndex_ == -1) {
        throw RuleViolation("No pending Confound discard selection.");
    }
    Player& player = playerByIndex(pendingConfoundDiscardPlayerIndex_);
    Card discarded = player.removeCardFromHand(handIndex);
    player.addToDiscard(std::move(discarded));
    pendingConfoundDiscardPlayerIndex_ = -1;
    endTurnIfNeeded();
