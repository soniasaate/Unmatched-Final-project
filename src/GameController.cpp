...
json pendingMovementChoiceToJson(const PendingMovementChoice& choice) {
    ...
    j["defenderId"] = choice.defenderId;
...
PendingMovementChoice jsonToPendingMovementChoice(const json& j) {
    ...
    choice.defenderId = j.value("defenderId", "");
...
void GameController::startNewGame(int playerOneAge, int playerTwoAge,
    ...
    for (int idx = 0; idx < 2; ++idx) {
        if (auto* invisible = dynamic_cast<InvisibleMan*>(players_.at(idx).fighters()[0].get())) {
...
void GameController::resolveAttack(const std::string& attackerId,
    ...
    try {
        ...
        auto appliesBeforeDamage = [](const Card& card) {
            return card.getTiming() == Timing::Immediately || card.getTiming() == Timing::DuringCombat;
        };

        if (defenseCard.has_value() && !defenseEffectsCanceled && appliesBeforeDamage(*defenseCard)) {
            defenseCard->applyEffect(defender, attacker, *this, defenseValue, attackValue);
        }
        if (!attackEffectsCanceled && appliesBeforeDamage(attackCard)) {
...
void GameController::playScheme(int handIndex, const SchemeChoice& choice) {
    ...
    try {
        ...
        if (card.getTitle() == "RAVENING SEDUCTION") {
            PendingRaveningChoice prChoice;
            prChoice.handIndex = handIndex;
            pendingRaveningChoice_ = prChoice;
            return;
        }
...
void GameController::finishRaveningScheme() {
    ...
    for (const auto& pair : pendingRaveningChoice_->newPositions) {
        Fighter* fighter = findFighterById(pair.first);
        if (fighter && !fighter->defeated()) {
            fighter->placeAt(pair.second);
        }
    }
...
std::vector<int> GameController::pendingOptionalMovementDestinations() const {
    ...
    if (pending.source == "THIRST") {
        return getThirstDestinations(pending.defenderId);
    }
...
void GameController::advanceTurn() {
    ...
    if (pendingVanishedPlacement_ && pendingVanishedPlayerIndex_ == currentPlayerIndex_) {
...
void GameController::queueOptionalMovement(int playerIndex,
    ...
    for (const auto& existing : pendingOptionalMovements_) {
        if (existing.fighterId == fighterId) return;
    }
...
void GameController::resolveCombatEffectAfterDamage(const Card& card,
    ...
    if (title == "CONFOUND") {
        if (dynamic_cast<InvisibleMan*>(&cardPlayer.heroFighter())) {
            ...
            choice.invisiblePlayerIndex = cardPlayer.id();
            choice.opponentDecided = false;
            pendingConfoundChoice_ = choice;
        }
        return;
    }
    if (title == "CODED NOTES") {
        for (int i = 0; i < 3; ++i) {
            drawCard(cardPlayer);
        }
    ...
    if (title == "COVERT PREPARATION") {
        drawCard(cardPlayer);
        auto* invisible = dynamic_cast<InvisibleMan*>(&cardFighter);
        if (!invisible) return;
        for (int token : invisible->getFogTokens()) {
            if (token != -1) {
                queueFogChoice(cardPlayer.id(), invisible->id(), 2, -1, "COVERT_PREPARATION_PLAYER");
                break;
            }
        }
        return;
    }
    if (title == "DREAMING OF REVENGE") {
        auto* invisible = dynamic_cast<InvisibleMan*>(&cardPlayer.heroFighter());
        if (!invisible || !invisible->isOnFog(invisible->spaceId())) return;
        for (auto& fighter : opposingPlayer.fighters()) {
    ...
    if (title == "INTO THIN AIR") {
        auto* invisible = dynamic_cast<InvisibleMan*>(&cardFighter);
        if (!invisible) return;
        queueOptionalMovement(cardPlayer.id(), cardFighter.id(), 1, "INTO THIN AIR");
        queueFogChoice(cardPlayer.id(), invisible->id(), 3, -1, "INTO THIN AIR");
        return;
    }
}
...
std::vector<int> GameController::pendingFogChoices() const {
    ...
    for (int i = 0; i < static_cast<int>(tokens.size()); ++i) {
        if (tokens[i] != -1 && i != pending.excludedIndex) {
            result.push_back(i);
        }
    }
    return result;
...
void GameController::moveFogToken(int fogIndex, int destinationSpace) {
    ...
    if (pending.source == "COVERT_PREPARATION_PLAYER") {
        bool hasOtherFog = false;
        for (int i = 0; i < 3; ++i) {
            if (i != fogIndex && invisible->getFogTokens()[i] != -1) {
                hasOtherFog = true;
                break;
            }
        }
        if (hasOtherFog) {
            int opponentIndex = (pending.chooserPlayerIndex == 0 ? 1 : 0);
            PendingFogChoice opponentChoice;
            opponentChoice.chooserPlayerIndex = opponentIndex;
            opponentChoice.fighterOwnerIndex = pending.fighterOwnerIndex;
            opponentChoice.fighterId = pending.fighterId;
            opponentChoice.maxSteps = 2;
            opponentChoice.excludedIndex = fogIndex;
            opponentChoice.source = "COVERT_PREPARATION_OPPONENT";
...
void GameController::queueFogChoice(int chooserPlayerIndex, const std::string& fighterId, int maxSteps, int excludedIndex, const std::string& source) {
    ...
    choice.fighterId = fighterId;
    choice.maxSteps = maxSteps;
    choice.excludedIndex = excludedIndex;
    choice.source = source;
...
void GameController::resolveConfoundFogMove(int fogIndex, int destinationSpace) {
    ...
    auto validDestinations = getValidConfoundDestinations(fogIndex);
    if (std::find(validDestinations.begin(), validDestinations.end(), destinationSpace) == validDestinations.end()) {
        throw RuleViolation("Invalid destination for confound fog move.");
    }

    int invisiblePlayerIndex = pendingConfoundChoice_->invisiblePlayerIndex == -1
        ? currentPlayerIndex_
        : pendingConfoundChoice_->invisiblePlayerIndex;
    auto* invisible = dynamic_cast<InvisibleMan*>(&playerByIndex(invisiblePlayerIndex).heroFighter());
...
std::vector<int> GameController::getValidConfoundDestinations(int fogIndex) const {
    ...
    int invisiblePlayerIndex = pendingConfoundChoice_->invisiblePlayerIndex == -1
        ? currentPlayerIndex_
        : pendingConfoundChoice_->invisiblePlayerIndex;
    const auto* invisible = dynamic_cast<const InvisibleMan*>(&playerByIndex(invisiblePlayerIndex).heroFighter());
    if (!invisible) return {};

    const auto& tokens = invisible->getFogTokens();
    if (fogIndex < 0 || fogIndex >= static_cast<int>(tokens.size()) || tokens[fogIndex] == -1) return {};

    int currentSpace = tokens[fogIndex];
    std::vector<int> result;

    for (const auto& space : board_.spaces()) {
        int spaceId = space.id();
        if (spaceId == currentSpace) continue;
        bool occupiedByOtherFog = false;
        for (int i = 0; i < static_cast<int>(tokens.size()); ++i) {
            if (i == fogIndex) continue;
            if (tokens[i] == spaceId) { occupiedByOtherFog = true; break; }
        }
...
bool GameController::hasPendingCodedNotes() const {
    return pendingCodedNotesChoice_.has_value();
}

const PendingCodedNotesChoice& GameController::pendingCodedNotes() const {
    if (!pendingCodedNotesChoice_.has_value()) {
        throw RuleViolation("No pending coded notes choice.");
    }
    return *pendingCodedNotesChoice_;
}

void GameController::cancelCodedNotes() {
    if (pendingCodedNotesChoice_.has_value()) {
        pendingCodedNotesChoice_.reset();
        endTurnIfNeeded();
    }
...
void GameController::finishCodedNotes(const std::vector<int>& selectedIndices) {
    if (!pendingCodedNotesChoice_.has_value()) {
        throw RuleViolation("No pending coded notes choice.");
    }
    ...
    Player& player = playerByIndex(pendingCodedNotesChoice_->playerIndex);
    ...
    for (int idx : sortedIndices) {
        ...
        Card removed = player.removeCardFromHand(idx);
        cardsToTop.push_back(std::move(removed));
    }

    for (auto it = cardsToTop.rbegin(); it != cardsToTop.rend(); ++it) {
        player.deck().push_back(std::move(*it));
    }
...
void GameController::handleVanish(int handIndex) {
    ...
    try {
        ...
        pendingVanishedPlayerIndex_ = currentPlayerIndex_;
        currentPlayer().addToDiscard(std::move(card));
...
void GameController::placeVanishedInvisibleMan(int spaceId) {
    if (!hasPendingVanishedPlacement()) {
...
json GameController::createFullSnapshot() const {
    ...
    j["pendingVanishedPlayerIndex"] = pendingVanishedPlayerIndex_;
    ...
    for (const auto& movement : pendingOptionalMovements_) {
        ...
        entry["defenderId"] = movement.defenderId;
...
void GameController::restoreFullSnapshot(const json& j) {
    ...
    pendingVanishedPlayerIndex_ = j.value("pendingVanishedPlayerIndex", -1);
    ...
    if (j.contains("pendingOptionalMovements")) {
        for (const auto& entry : j["pendingOptionalMovements"]) {
            ...
            choice.defenderId = entry.value("defenderId", "");
    ...
    if (pendingVanishedPlacement_ && pendingVanishedPlayerIndex_ < 0) {
        for (const auto& player : players_) {
            const Fighter& hero = player.heroFighter();
            if (dynamic_cast<const InvisibleMan*>(&hero) != nullptr && hero.spaceId() == -1) {
                pendingVanishedPlayerIndex_ = player.id();
                break;
            }
        }
        if (pendingVanishedPlayerIndex_ < 0 &&
            currentPlayerIndex_ >= 0 &&
            currentPlayerIndex_ < static_cast<int>(players_.size()) &&
            dynamic_cast<const InvisibleMan*>(&currentPlayer().heroFighter()) != nullptr) {
            pendingVanishedPlayerIndex_ = currentPlayerIndex_;
        }
    }
}
...
std::vector<int> GameController::getLurkingFogPositions() const {
    ...
    const Fighter& hero = playerByIndex(pendingLurkingChoice_->playerIndex).heroFighter();
...
void GameController::resolveLurkingDestination(int destinationSpace) {
    ...
    Player& player = playerByIndex(pendingLurkingChoice_->playerIndex);
...
std::vector<int> GameController::getSlipAwayFogTokens() const {
    if (!pendingSlipAwayChoice_.has_value()) return {};
    const auto* invisible = dynamic_cast<const InvisibleMan*>(&playerByIndex(pendingSlipAwayChoice_->playerIndex).heroFighter());
    if (!invisible) return {};
    std::vector<int> res;
    for (int i = 0; i < 3; ++i) {
...
void GameController::resolveSlipAwayDestination(int destinationSpace) {
    ...
    auto* invisible = dynamic_cast<InvisibleMan*>(&playerByIndex(pendingSlipAwayChoice_->playerIndex).heroFighter());
